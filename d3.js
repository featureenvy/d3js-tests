!function() {
  ...
  function d3_map_remove(key) {
    return (key = d3_map_escape(key)) in this._ &&
      delete this._[key];
  }
  ...
  d3.nest = function() {
    var nest = {},
      keys = [],
      sortKeys = [], sortValues, rollup;
    function map(mapType, array, depth) {
      if (depth >= keys.length) return rollup ? rollup.call(nest, array) : sortValues ? array.sort(sortValues) : array;
      var i = -1,
        n = array.length,
        key = keys[depth++], keyValue, object, setter,
  ...
  d3_selectionPrototype.data = function(value, key) {
    ...
    function bind(group, groupData) {
      var i,
        ...
        exitNodes = new Array(n), node, nodeData;
      if (key) {
  ...
  d3.behavior.zoom = function() {
    var view = {
      ...
      mouseup = "mouseup.zoom", mousewheelTimer,
      touchstart = "touchstart.zoom", touchtime,
  ...
  function d3_geo_streamLine(coordinates, listener, closed) {
    var i = -1,
      n = coordinates.length - closed, coordinate;
    listener.lineStart();
  ...
  d3.geo.bounds = function() {
    ...
    var bound = {
      ...
      polygonEnd: function() {
        ...
        if (d3_geo_areaRingSum < 0) λ0 = -(λ1 = 180), φ0 = -(φ1 = 90); else if (dλSum > ε)
  ...
  function d3_geo_clip(pointVisible, clipLine, interpolate, clipStart) {
    return function(rotate, listener) {
      ...
      var buffer = d3_geo_clipBufferListener(),
        ...
        polygonStarted = false, polygon, ring;
      ...
      function ringEnd() {
        ...
        var clean = ringListener.clean(),
          ringSegments = buffer.buffer(), segment,
  ...
  d3.geo.circle = function() {
    var origin = [0, 0], angle,
      precision = 6, interpolate;
    function circle() {
  ...
  function d3_geom_voronoiAddBeach(site) {
    var x = site.x,
      directrix = site.y, lArc, rArc, dxl, dxr,
  ...
  d3_geom_voronoiCell.prototype.prepare = function() {
    var halfEdges = this.edges,
      iHalfEdge = halfEdges.length, edge;
    while (iHalfEdge--) {
  ...
  function d3_geom_voronoiCloseCells(extent) {
    var x0 = extent[0][0],
      ...
      iCell = cells.length, cell, iHalfEdge, halfEdges, nHalfEdges, start, end;
    while (iCell--) {
      cell = cells[iCell];
  ...
  d3.geom.quadtree = function(points, x1, y1, x2, y2) {
    ...
    function quadtree(data) {
      var d,
        fx = d3_functor(x),
        fy = d3_functor(y), xs, ys, i, n, x1_, y1_, x2_, y2_;
      if (x1 != null) {
    ...
    quadtree.extent = function(_) {
      ...
      if (_ == null)
        x1 = y1 = x2 = y2 = null; else x1 = +_[0][0], y1 = +_[0][1], x2 = +_[1][0],
        y2 = +_[1][1];
      return quadtree;
    };
    quadtree.size = function(_) {
      ...
      if (_ == null)
        x1 = y1 = x2 = y2 = null; else x1 = y1 = 0, x2 = +_[0], y2 = +_[1];
      return quadtree;
    };
    return quadtree;
  };
  ...
  function d3_interpolateString(a, b) {
    var bi = d3_interpolate_numberA.lastIndex = d3_interpolate_numberB.lastIndex = 0, am, bm, bs,
      i = -1,
      s = [],
      q = [];
    a = a + "", b = b + "";
  ...
  function d3_ease_elastic(a, p) {
    ...
    if (arguments.length < 2)
      p = .45;
    if (arguments.length)
      s = p / τ * Math.asin(1 / a); else a = 1, s = p / 4;
    return function(t) {
  ...
  function d3_interpolateHcl(a, b) {
    ...
    if (isNaN(bh)) bh = 0, ah = isNaN(ah) ? b.h : ah; else if (bh > 180)
  ...
  d3.layout.chord = function() {
    var chord = {}, chords, groups, matrix, n,
      padding = 0, sortGroups, sortSubgroups, sortChords;
    function relayout() {
      var subgroups = {},
        groupSums = [],
  ...
  d3.layout.force = function() {
    ...
    force.tick = function() {
      ...
      var n = nodes.length,
        m = links.length, q, i, o, s, t, l, k, x, y;
      for (i = 0; i < m; ++i) {
        o = links[i];
        s = o.source;
        t = o.target;
  ...
  var d3_layout_stackOffsets = d3.map({
    ...
    wiggle: function(data) {
      var n = data.length,
        x = data[0],
        m = x.length, i, j, k, s1, s2, s3, dx, o, o0,
        y0 = [];
      y0[0] = o = o0 = 0;
      for (j = 1; j < m; ++j) {
  ...
  d3.layout.histogram = function() {
    ...
    function histogram(data, i) {
      var bins = [],
        ...
        thresholds = binner.call(this, range, values, i), bin,
        i = -1,
        n = values.length,
  ...
  d3_transitionPrototype.select = function(selector) {
    var id = this.id,
      ns = this.namespace,
      subgroups = [], subgroup, subnode, node;
    selector = d3_selection_selector(selector);
    for (var j = -1, m = this.length; ++j < m;) {
      subgroups.push(subgroup = []);
      for (var group = this[j], i = -1, n = group.length; ++i < n;) {
  ...
  d3_transitionPrototype.transition = function() {
    var id0 = this.id,
      id1 = ++d3_transitionId,
      ns = this.namespace,
      subgroups = [], subgroup, group, node, transition;
    for (var j = 0, m = this.length; j < m; j++) {
      subgroups.push(subgroup = []);
      for (var group = this[j], i = 0, n = group.length; i < n; i++) {
        if (node = group[i]) {
  ...
  function d3_transitionNode(node, i, ns, id, inherit) {
    ...
    if (!transition) {
      ...
      d3.timer(function(elapsed) {
        var delay = transition.delay, duration, ease,
  ...
  d3.svg.brush = function() {
    var event = d3_eventDispatch(brush, "brushstart", "brush", "brushend"),
      ...
      y = null,
      xExtent = [0, 0],
      yExtent = [0, 0], xExtentDomain, yExtentDomain,
      xClamp = true,
      yClamp = true,
    ...
    function brush(g) {
      g.each(function() {
        ...
        var gUpdate = d3.transition(g),
          backgroundUpdate = d3.transition(background), range;
        if (x) {
    ...
    brush.clamp = function(z) {
      ...
      if (x && y) xClamp = !!z[0], yClamp = !!z[1]; else if (x)
        xClamp = !!z; else if (y)
        yClamp = !!z;
      return brush;
    };
  ...
  if (typeof define === "function" && define.amd) define(d3); else if (typeof module === "object" && module.exports)
    module.exports = d3;
  this.d3 = d3;
}();
